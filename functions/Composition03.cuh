...
template<> 
inline void Composition03<double>::transpose_fit(){
    double alpha = 1.0;
    double beta  = 0.0;
...
template<> 
inline void Composition03<float>::transpose_fit(){
    float alpha = 1.0;
    float beta  = 0.0;
...
__global__ void griewank_gpu(T *x, T *f, int nx){
    ...
    if(threadIdx.x == 0){
        ...
        f[chromo_id] = term1/4000.0 - term2 + 1;
    }
}
#endif
